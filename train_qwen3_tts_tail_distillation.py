...
ACLGRAPH_IMPLEMENTATION = "prefix_reprefill_v1"
ACLGRAPH_HIDDEN_SOURCE = "code_predictor_aclgraph_reprefill"
...
class _ACLGraphStep:
    """One captured Code Predictor prefix step and its static outputs."""
...
@dataclasses.dataclass
class _ACLGraphBucket:
    """Persistent tensors and graphs for one padded rollout batch."""

    static_inputs: torch.Tensor
    projection_buffer: torch.Tensor
    position_ids: dict[int, torch.Tensor]
    cache_positions: dict[int, torch.Tensor]
    initialize_graph: Any
    steps: list[_ACLGraphStep]


class TeacherTraceACLGraphRunner:
    """Run the official Teacher Code Predictor through prefix ACL Graphs."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a lazy graph runner.

        Args:
            code_predictor: Frozen official qwen-tts Code Predictor.
            truncation_k: First RVQ codebook generated by the Student.
            batch_size: Static graph batch; shorter final batches are padded.
            warmups: Number of eager prefix warmups before capture.
        """
        self.code_predictor = code_predictor
        self.truncation_k = truncation_k
        self.batch_size = batch_size
        self.warmups = warmups
        self.num_groups = int(code_predictor.config.num_code_groups)
        self.model_dtype = next(code_predictor.model.parameters()).dtype
        self._pool = torch.npu.graph_pool_handle()
        self._bucket: _ACLGraphBucket | None = None
        self._validated = False
        self.generate_calls = 0
        self.graph_replays = 0

    def _initialize(self, bucket: _ACLGraphBucket) -> None:
        """Project the static Talker/codec0 inputs into the CP buffer."""
        projected = self.code_predictor.small_to_mtp_projection(
            bucket.static_inputs
        )
        bucket.projection_buffer.zero_()
        bucket.projection_buffer[:, :2].copy_(projected)

    def _step(
        self,
        bucket: _ACLGraphBucket,
        step: int,
        position_ids: torch.Tensor,
        cache_position: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Execute one greedy re-prefill step and populate its successor."""
        outputs = self.code_predictor.model(
            inputs_embeds=bucket.projection_buffer[:, : step + 1],
            position_ids=position_ids,
            cache_position=cache_position,
            use_cache=False,
            output_hidden_states=False,
        )
        hidden = outputs.last_hidden_state[:, step]
        logits = self.code_predictor.lm_head[step - 1](hidden)
        code = logits.argmax(dim=-1)
        if step < self.num_groups - 1:
            embedding = self.code_predictor.get_input_embeddings()[step - 1](code)
            projected = self.code_predictor.small_to_mtp_projection(
                embedding.unsqueeze(1)
            ).squeeze(1)
            bucket.projection_buffer[:, step + 1].copy_(projected)
        return code, hidden

    def _capture(self, inputs_embeds: torch.Tensor) -> _ACLGraphBucket:
        """Warm up and capture initialization plus all residual CP steps."""
        if inputs_embeds.ndim != 3 or inputs_embeds.shape[1] != 2:
            raise ValueError(
                "ACL Graph Code Predictor expects inputs_embeds shaped [batch, 2, hidden]."
            )
        device = inputs_embeds.device
        input_hidden_size = int(inputs_embeds.shape[-1])
        ...
        projection_buffer = torch.zeros(
            self.batch_size,
            self.num_groups,
            int(self.code_predictor.config.hidden_size),
            dtype=self.model_dtype,
            device=device,
        )
        bucket = _ACLGraphBucket(
        ...
        for step in range(1, self.num_groups):
            bucket.position_ids[step] = (
                torch.arange(step + 1, device=device, dtype=torch.long)
                .unsqueeze(0)
                .expand(self.batch_size, -1)
                .contiguous()
            )
            bucket.cache_positions[step] = torch.arange(
                step + 1,
                device=device,
                dtype=torch.long,
            )
        ...
        for _ in range(self.warmups):
            self._initialize(bucket)
            for step in range(1, self.num_groups):
                self._step(
                    bucket,
                    step,
                    bucket.position_ids[step],
                    bucket.cache_positions[step],
                )
        torch.npu.synchronize()
        warmup_seconds = time.perf_counter() - started_at

        capture_started_at = time.perf_counter()
        initialize_graph = torch.npu.NPUGraph()
        with torch.npu.graph(initialize_graph, pool=self._pool):
            self._initialize(bucket)
        bucket.initialize_graph = initialize_graph
        for step in range(1, self.num_groups):
            graph = torch.npu.NPUGraph()
            with torch.npu.graph(graph, pool=self._pool):
                code, hidden = self._step(
                    bucket,
                    step,
                    bucket.position_ids[step],
                    bucket.cache_positions[step],
                )
            bucket.steps.append(_ACLGraphStep(graph=graph, code=code, hidden=hidden))
        torch.npu.synchronize()
        capture_seconds = time.perf_counter() - capture_started_at
        print(
            json.dumps(
                {
                    "record_type": "teacher_trace_aclgraph_captured",
                    "batch_size": self.batch_size,
                    "graphs": 1 + len(bucket.steps),
                    "prefix_lengths": list(range(2, self.num_groups + 1)),
    ...
    def generate(self, *, inputs_embeds: torch.Tensor, **kwargs: Any) -> Any:
        """Replay captured graphs and return HF-compatible residual sequences."""
        batch_size = int(inputs_embeds.shape[0])
        if batch_size > self.batch_size:
            raise ValueError(
                f"ACL Graph batch {self.batch_size} cannot serve batch {batch_size}."
            )
        if bool(kwargs.get("do_sample", False)):
            raise ValueError("Teacher trace ACL Graph currently supports greedy CP only.")
        expected_tokens = self.num_groups - 1
        if int(kwargs.get("max_new_tokens", expected_tokens)) != expected_tokens:
            raise ValueError(
                f"ACL Graph CP must generate exactly {expected_tokens} residual codecs."
            )
        if self._bucket is None:
            self._bucket = self._capture(inputs_embeds)
        bucket = self._bucket
        if inputs_embeds.shape[-1] != bucket.static_inputs.shape[-1]:
            raise ValueError("Talker hidden size changed after ACL Graph capture.")

        bucket.static_inputs.zero_()
        bucket.static_inputs[:batch_size].copy_(
            inputs_embeds.to(dtype=self.model_dtype)
        )
        bucket.initialize_graph.replay()
        codes: list[torch.Tensor] = []
        selected_hidden: list[torch.Tensor] = []
        for step, graph_step in enumerate(bucket.steps, start=1):
            graph_step.graph.replay()
            # Graphs share one pool, so static outputs may alias across prefix
            # graphs. Preserve each step before replaying the next graph.
            codes.append(graph_step.code[:batch_size].clone())
            if step >= self.truncation_k - 1:
                selected_hidden.append(graph_step.hidden[:batch_size].clone())

        sequences = torch.stack(codes, dim=1)
        trace_hidden = torch.stack(selected_hidden, dim=1)
        if not self._validated:
            ...
            eager_sequences = torch.stack(eager_codes, dim=1)
            if not torch.equal(sequences, eager_sequences):
                agreement = float((sequences == eager_sequences).float().mean().cpu())
                raise RuntimeError(
                    "Teacher trace ACL Graph replay diverged from eager re-prefill "
                    f"(residual_top1={agreement:.6f})."
                )
            self._validated = True
            print(
                json.dumps(
                    {
                        "record_type": "teacher_trace_aclgraph_validation",
                        "batch_size": batch_size,
                        "residual_code_exact": True,
                    },
                    sort_keys=True,
                ),
                flush=True,
        ...
        self.graph_replays += 1 + len(bucket.steps)
        return SimpleNamespace(
            sequences=sequences,
            teacher_trace_hidden=trace_hidden,
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--teacher-trace-aclgraph",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Capture and replay fixed-prefix NPU graphs for Teacher Code Predictor rollouts.",
    )
    parser.add_argument("--teacher-trace-aclgraph-warmups", type=int, default=3)
    parser.add_argument(
        "--batch-size",
        type=int,
        default=4096,
...
def _trace_fingerprint(
    ...
    *,
    ...
    rollout_config = {
        ...
        "teacher_trace_aclgraph": args.teacher_trace_aclgraph,
        "teacher_trace_aclgraph_warmups": args.teacher_trace_aclgraph_warmups,
        "teacher_trace_aclgraph_implementation": ACLGRAPH_IMPLEMENTATION,
...
def _generate_teacher_rollout(
    ...
    rows: Sequence[SeedTTSRow],
    args: argparse.Namespace,
    graph_runner: TeacherTraceACLGraphRunner | None,
    ...
    def capture_generate(*generate_args: Any, **generate_kwargs: Any) -> Any:
        if graph_runner is None:
            ...
            if generate_args:
                raise ValueError("ACL Graph CP requires keyword generation arguments.")
            predictor_result = graph_runner.generate(**generate_kwargs)
        captured_residual_codes.append(predictor_result.sequences.detach())
        graph_hidden = getattr(predictor_result, "teacher_trace_hidden", None)
        if graph_hidden is None:
            ...
            captured_frames.append(graph_hidden.detach())
    ...
    *,
    ...
    save_file(
        ...
        metadata={
            ...
            "hidden_source": (
                ACLGRAPH_HIDDEN_SOURCE
                if graph_runner is not None
                else "code_predictor_generation"
            ),
            "target_audio_used": "false",
        },
    )
...
def _validate_aclgraph_runtime() -> None:
    """Require the torch-npu capture/replay APIs used by Teacher traces."""
    npu = getattr(torch, "npu", None)
    required = ("NPUGraph", "graph", "graph_pool_handle", "synchronize")
    missing = [name for name in required if npu is None or not hasattr(npu, name)]
    if missing:
        raise RuntimeError(
            "Teacher trace ACL Graph requires torch-npu APIs: " + ", ".join(missing)
        )
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    positive_ints = (
        "rollout_batch_size",
        "teacher_trace_aclgraph_warmups",
        "batch_size",
    ...
    if args.teacher_trace_aclgraph:
        if not str(args.device).startswith("npu"):
            raise ValueError("teacher-trace-aclgraph requires an NPU device.")
        if args.rollout_subtalker_do_sample:
            raise ValueError(
                "teacher-trace-aclgraph currently requires --no-rollout-subtalker-do-sample."
            )
...
def main() -> int:
    ...
    graph_runner = (
        TeacherTraceACLGraphRunner(
            code_predictor,
            truncation_k=args.truncation_k,
            batch_size=args.rollout_batch_size,
            warmups=args.teacher_trace_aclgraph_warmups,
        )
        if args.teacher_trace_aclgraph
        else None
    )
    print(
        json.dumps(
            {
                "record_type": "teacher_trace_aclgraph_config",
                "enabled": graph_runner is not None,
                "implementation": (
                    ACLGRAPH_IMPLEMENTATION
                    if graph_runner is not None
                    else "hf_dynamic_cache"
                ),
                "batch_size": args.rollout_batch_size,
                "warmups": args.teacher_trace_aclgraph_warmups,
            },
    ...
    graph_enabled = graph_runner is not None
    graph_generate_calls = graph_runner.generate_calls if graph_runner is not None else 0
    graph_replays = graph_runner.graph_replays if graph_runner is not None else 0
    trace_hidden_source = (
        ACLGRAPH_HIDDEN_SOURCE if graph_enabled else "code_predictor_generation"
    )
    if graph_enabled:
        torch.npu.synchronize()
    del graph_runner, code_predictor, teacher, wrapper
    gc.collect()
        ...
        json.dumps(
            {
                ...
                "teacher_trace_aclgraph": graph_enabled,
                "teacher_trace_aclgraph_generate_calls": graph_generate_calls,
                "teacher_trace_aclgraph_replays": graph_replays,
    ...
    training_record = {
        ...
        "teacher_trace_hidden_source": trace_hidden_source,
        "teacher_trace_aclgraph": graph_enabled,
        "teacher_trace_aclgraph_generate_calls": graph_generate_calls,
        "teacher_trace_aclgraph_replays": graph_replays,
        "training_loop_teacher_forwards": 0,
