...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--seed",
        type=int,
        default=int(os.environ.get("QWEN3_TTS_BENCH_SEED", "0")),
    ...
    parser.add_argument(
        "--seed-tts-utmos-jit-file",
        default=None,
        help="UTMOS JIT filename inside the HF repo/directory. Defaults to utmos.jit.",
    )
...
def _summary_row(label: str, result_path: Path, result: dict[str, Any], *, seed: int) -> dict[str, Any]:
    return {
        "label": label,
        "benchmark_seed": seed,
...
def main() -> int:
    ...
    if args.seed_tts_utmos_model:
        utmos_model = Path(args.seed_tts_utmos_model).expanduser()
        if utmos_model.is_file():
            extra_env["SEED_TTS_UTMOS_JIT_PATH"] = str(utmos_model.resolve())
        else:
            extra_env["SEED_TTS_UTMOS_HF_REPO"] = args.seed_tts_utmos_model
    if args.seed_tts_utmos_jit_file:
        extra_env["SEED_TTS_UTMOS_JIT_FILE"] = args.seed_tts_utmos_jit_file
